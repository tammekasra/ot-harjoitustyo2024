...
class TestPrintSudokuBoard(unittest.TestCase):
    ...
    def test_if_there_are_no_zeros_in_grid(self):
        # we dont need to do the tests via main.... stupid me
    ...
    # sudoku_board = main.SudokuGame.sudoku_board
        ...
        for i in range(9):
            for j in range(9):
                self.assertNotEqual(
                    sudoku_board[i][j], 0, "There should be no zeros in the grid")
    ...
    def test_if_there_are_same_numbers_on_the_same_row(self):
        ...
        for i in range(9):
            ...
            for j in range(9):  # we check the columns
                column = row[j]
                # We don't want that the number we are checking is the same as in the rows!
                for current_number in range(9):
                    if current_number != j:
                        # checks if the numbers in the row are not the same!
                        self.assertNotEqual(column, row[current_number])
    ...
    def test_if_there_are_same_numbers_on_the_same_3x3_grid(self):
        ...
        self.assertFalse(
            duplicates, "There are same numbers in the same 3x3 grid.")
