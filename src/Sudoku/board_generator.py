...
class BoardGenerator:
            ...
            for i in range(9):  # Need to remember that this is the ROW
                for j in range(9):  # Need to remember that this is the COLUMN
                    if sudoku_board[i][j] == 0:
                        # we just go through all the numbers from 1-10 to check if they can be put there
                        for random_number in random.sample(range(1, 10), 9):
    ...
    def valid(board, row, column, random_number):
        ...
        '''
        start_row = (
            row // 3) * 3  # This just takes the row number - 3 // 3 = 0 - we check row 1 and 4 // 3 = 1, we check row 2 and  7//3 = 2 - we check row 3
        # This just takes the column number - 3 // 3 = 0 - we check column 1 and 4 // 3 = 1, we check column 2 and  7//3 = 2 - we check column 3
